...
- `tools/list` → 7 个工具全部注册：`bind_csdn` / `create_article` / `update_article` / `publish_article` / `list_articles` / `delete_article` / `get_article`
...
验证：对真实文章 `163948772` 端到端抓取，`get_article` 返回已无「目录」、含两处正确 Markdown 表格、封面图正确降级；`go test ./...` 全过；`tools/list` 7 工具全部注册。
